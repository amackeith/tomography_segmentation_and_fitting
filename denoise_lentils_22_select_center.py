...
half_min_side_length = min([arr.shape[1],arr.shape[2]])/2.0 #the smaller of the non-verticle array sides
...
def onclick(event):
    ...
    if event.button == 1 and event.inaxes != None:
        ...
        plt.scatter(x,y, scale, facecolors='none', edgecolors='r') #inform matplotlib of the new data
        plt.draw() #redraw


    if event.button == 3:
        #right clicking gives you the option to say your are done, you want to remove a circle, or that it was a mistake
        done = {'d', 'done'}
        no = {'no', 'n'}
        remove = {'r', 'remove'}
        ...
        if choice in remove:
            ...
            plt.xlabel("e1")
            plt.ylabel("e2")
            plt.title("To approve, close the plot, else right click and select no")
            plt.scatter(best_x,best_y, [20,30], facecolors='none', edgecolors='r'); #inform matplotlib of the new data
            plt.draw() #redraw
...
def motion_notify(event):
    ...
    s = get_s()#scale for this particular instance
    s = np.pi*(s**2)
    #clear frame and plot new stuff this time including a hypothetical circle that may or may not be added
    plt.clf()
    plt.imshow(arr[slice_index], cmap='gray', origin='lower')
    plt.xlabel("e1")
    plt.ylabel("e2")
    plt.title("left click to add circles, (recomend using at least 5)\n right click to remove last circle or approve mean")
    plt.scatter([event.xdata], [event.ydata], s, facecolors='none', edgecolors='purple');#adds candidate cirlce
    plt.scatter(x,y, scale, facecolors='none', edgecolors='r'); #inform matplotlib of the new data
    plt.draw() #redraw
...
def handle_close(event):
    ...
    if finalized == False:
        sys.stdout.write("ERROR: Program not properly exited bad mean returned\n")

    elif len(x) == 0:
        sys.stdout.write("ERROR: No Circles have been selected, please select the center of the rings\n")
