...
if __name__ == '__main__':
	...
	q = Queue()
	results = []
	multi_core_on = mp.cpu_count() > 70
	if not multi_core_on:
		print("using single core as there were less than 7 cores to use")
		get_loc_and_angle(fitting,lbls[400:500],q,0)
		...
		p1 = Process(target=get_loc_and_angle, args=(fitting, lbl1, q, 1))
		p1.start()
		p2 = Process(target=get_loc_and_angle, args=(fitting, lbl2, q,2))
		p2.start()
		p3 = Process(target=get_loc_and_angle, args=(fitting, lbl3, q,3))
		p3.start()
		p4 = Process(target=get_loc_and_angle, args=(fitting, lbl4, q,4))
		p4.start()
		
		p5 = Process(target=get_loc_and_angle, args=(fitting, lbl5, q,5))
		p5.start()
		p6 = Process(target=get_loc_and_angle, args=(fitting, lbl6, q,6))
		p6.start()
