...
if len(sys.argv)==7:
	...
	threshold_of_edm2 = float(sys.argv[5]) #second threshold for determinin what is space what is not (and assigning height map)
	min_vol_for_segment = int(float(sys.argv[6])) #anything smaller than this is thrown out
	print "label_watershed_method.py params:", image_name_in, threshold_of_binary, threshold_of_eroded_binary, gauss_filt_sigma, threshold_of_edm2, min_vol_for_segment

else:
	print "wrong number of args", sys.argv
	print len(sys.argv)
	print "should be python pgrm.py volume_file threshold_of_binary threshold_of_eroded_binary gauss_filt_sigma threshold_of_edm2 min_vol_for_segment"
	exit()


paramiters = "_file_in_%s_binary_thresh_%s_erosion_thresh_%s_gauss_filt_sigma_%s_edmthresh_%s_min_vol_%s_" % \
			(image_name_in, threshold_of_binary, threshold_of_eroded_binary, gauss_filt_sigma, threshold_of_edm2,min_vol_for_segment)
...
rm_holes = np.int32((hoshen_np_image!=background_lbl)*1) #a threshold version with the holes filled in (important since we
...
plt.show()
min_vol_for_segment = int(float(raw_input("Min Single Lent Value")))
